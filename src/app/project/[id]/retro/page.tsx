    ...
    <AppShell>
      ...
      <div className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => router.push(`/project/${id}/retro/new`)}>+ 新建复盘</Button>
        </div>

        {retros.length === 0 ? (
          <EmptyState icon="pencil" title="暂无复盘记录" description="阶段结束时创建复盘，AI 辅助生成结构化报告" action={{ label: '新建复盘', onClick: () => router.push(`/project/${id}/retro/new`) }} />
        ) : (
          <div className="space-y-3">
            {retros.map(r => (
              <div
                key={r.id}
                onClick={() => router.push(`/project/${id}/retro/${r.id}`)}
