    ...
    <AppShell>
      ...
      <div className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => router.push(`/project/meetings/new?p=${id}`)}>+ 新建会议</Button>
        </div>

        {meetings.length === 0 ? (
          <EmptyState icon="microphone" title="暂无会议记录" description="记录会议内容，AI 自动提取行动项" action={{ label: '新建会议', onClick: () => router.push(`/project/meetings/new?p=${id}`) }} />
        ) : (
          <div className="space-y-3">
            {meetings.map(m => (
              <div
                key={m.id}
                onClick={() => router.push(`/project/meetings/detail?p=${id}&mid=${m.id}`)}
...
export default function MeetingsPage() {
  return <Suspense fallback={null}><MeetingsPageContent /></Suspense>;
}
