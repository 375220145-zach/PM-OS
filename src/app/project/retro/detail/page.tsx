...
function RetroDetailPageContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const projectId = searchParams.get('p') ?? '';
  const retroId = searchParams.get('rid') ?? '';
    ...
    <AppShell>
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button size="sm" onClick={() => router.push(`/project/retro/report?p=${projectId}&rid=${retroId}`)}>
            📄 导出复盘报告
          </Button>
        </div>
...
export default function RetroDetailPage() {
  return <Suspense fallback={null}><RetroDetailPageContent /></Suspense>;
}
