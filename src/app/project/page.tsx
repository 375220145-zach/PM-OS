    ...
    <AppShell>
      ...
      <div className="p-4 md:p-6 space-y-4 md:space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => router.push(`/project/schedule?p=${id}`)}>进入进度管理</Button>
          <Button variant="secondary" onClick={() => router.push(`/project/meetings/new?p=${id}`)}>新建会议记录</Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <RiskRow label="逾期任务" count={overdueTasks} color="red" onClick={() => router.push(`/project/schedule?p=${id}`)} />
              <RiskRow label="高风险任务" count={highRiskTasks.length} color="orange" onClick={() => router.push(`/project/schedule?p=${id}`)} />
              <RiskRow label="A 类 MIL" count={aClassMIL} color="red" onClick={() => router.push(`/project/mil?p=${id}`)} />
...
export default function ProjectOverviewPage() {
  return <Suspense fallback={null}><ProjectOverviewPageContent /></Suspense>;
}
