...
// index.ts
var TABLE_WHITELIST = /* @__PURE__ */ new Set([
  "projects",
  "milestones",
  "tasks",
  "meetings",
...
function authorized(request, env2) {
  if (!env2.CLIENT_SECRET) return true;
  return request.headers.get("X-Client-Secret") === env2.CLIENT_SECRET;
}
__name(authorized, "authorized");
async function pullHandler(env2) {
  const rows = await env2.DB.prepare("SELECT tbl, id, data FROM sync_rows").all();
  const meta = await env2.DB.prepare("SELECT v FROM sync_meta WHERE k = 'revision'").first();
  const grouped = {};
...
__name(pullHandler, "pullHandler");
async function pushHandler(env2, body) {
  ...
  for (const c of changes) {
    ...
    if (c.op === "del") {
      stmts.push(env2.DB.prepare("DELETE FROM sync_rows WHERE tbl = ? AND id = ?").bind(c.t, c.k));
    } else {
      stmts.push(
        env2.DB.prepare(
  ...
  stmts.push(env2.DB.prepare("INSERT INTO sync_meta (k, v) VALUES ('revision', '0') ON CONFLICT(k) DO NOTHING"));
  stmts.push(env2.DB.prepare("UPDATE sync_meta SET v = CAST(v AS INTEGER) + 1 WHERE k = 'revision'"));
  await env2.DB.batch(stmts);
  const rev = await env2.DB.prepare("SELECT v FROM sync_meta WHERE k = 'revision'").first();
  return json({ ok: true, revision: Number(rev?.v ?? 0) });
}
__name(pushHandler, "pushHandler");
async function chatHandler(env2, body) {
  ...
  async fetch(request, env2) {
    ...
    try {
      if (url.pathname === "/api/sync/pull" && request.method === "GET") {
        return await pullHandler(env2);
      }
      if (url.pathname === "/api/sync/push" && request.method === "POST") {
        return await pushHandler(env2, await request.json());
      }
      if (url.pathname === "/api/ai/chat" && request.method === "POST") {
        return await chatHandler(env2, await request.json());
      }
    } catch (err) {
...
// .wrangler/tmp/bundle-3MFuaN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3MFuaN/middleware-loader.entry.ts
